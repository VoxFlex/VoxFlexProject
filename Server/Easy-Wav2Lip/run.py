...
video_file = args.video_file if args.video_file else config['OPTIONS'].get('video_file', "").strip()
vocal_file = args.vocal_file if args.vocal_file else config['OPTIONS'].get('vocal_file', "").strip()
output_file = args.output_file if args.output_file else config['OPTIONS'].get('output_file', "").strip()

if not video_file:
    sys.exit("Error: `video_file` is not defined. Provide it as an argument or in `config.ini`.")
if not output_file:
    output_file = os.path.join(os.getcwd(), "output.mp4")  # Default output path
...
while True:
    ...
    output_video = output_file if output_file else os.path.join(folder, output_filename + output_suffix + ".mp4")
        ...
        with open(os.devnull, "w") as devnull:
            with contextlib.redirect_stdout(devnull), contextlib.redirect_stderr(
                ...
                subprocess.call([
                    ...
                    "-ss", "0",  # Start time
                    "-to", str(audio_length),  # End time based on audio length
